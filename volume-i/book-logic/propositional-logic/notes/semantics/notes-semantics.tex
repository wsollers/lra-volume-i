\subsection*{Truth Assignments}
\label{subsec:truth-assignments-propositional-logic}

% BEGIN GENERATED ARTIFACT: def:truth-assignment-propositional-logic
\begin{definitionbox}{Definition (Truth Assignment)}
\begin{definition}[Truth Assignment]
\label{def:truth-assignment-propositional-logic}
A \emph{truth assignment} is a function
\[
v:\Prop\to\mathbb{B}.
\]
For each propositional variable \(P\in\Prop\), the value \(v(P)\) is the truth
value assigned to \(P\).
\end{definition}
\LeanFormalizes{def:truth-assignment-propositional-logic}{lra-lean}{LRA.VolumeI.Logic.Model.Propositional.PropositionalModel}{PropositionalModel}{pending}
\end{definitionbox}

\begin{remark*}[Standard quantified statement]
\[
v\in\mathsf{TruthAssignment}
\Longleftrightarrow
v:\Prop\to\mathbb{B}.
\]
\end{remark*}

\begin{remark*}[Predicate reading]
\[
v\in\mathsf{TruthAssignment}
\]
means that \(v\) assigns exactly one truth value to each propositional variable.
\end{remark*}

\begin{remark*}[Interpretation]
A truth assignment gives semantic values to the atomic formulas. The recursive
truth conditions for the connectives then determine the truth value of every
well-formed formula.
\end{remark*}

\begin{remark*}[Examples]
If \(\Prop=\{P_1,P_2,P_3,\ldots\}\), then a truth assignment may satisfy
\[
v(P_1)=\True,\qquad v(P_2)=\False,\qquad v(P_3)=\True,
\]
with values assigned to every remaining propositional variable.
\end{remark*}

\begin{remark*}[Non-Examples]
A partial assignment that gives values only to \(P_1\) and \(P_2\) is not a truth
assignment on \(\Prop\). It may be useful for local computations, but it is not a
function \(\Prop\to\mathbb{B}\).
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:truth-value-propositional-logic]{Truth Value}
  \item \hyperref[def:propositional-variable]{Propositional Variable}
  \item \hyperref[def:well-formed-formula]{Well-Formed Formula}
\end{itemize}
\end{dependencies}
% END GENERATED ARTIFACT: def:truth-assignment-propositional-logic

% BEGIN GENERATED ARTIFACT: def:truth-assignment-domain-propositional-logic
\begin{definitionbox}{Definition (Domain of a Truth Assignment)}
\begin{definition}[Domain of a Truth Assignment]
\label{def:truth-assignment-domain-propositional-logic}
The \emph{domain} of a truth assignment
\[
v:\Prop\to\mathbb{B}
\]
is the set \(\Prop\) of propositional variables.
\end{definition}
\end{definitionbox}

\begin{remark*}[Standard quantified statement]
\[
v\in\mathsf{TruthAssignment}\Longrightarrow \operatorname{dom}(v)=\Prop.
\]
\end{remark*}

\begin{remark*}[Interpretation]
A truth assignment is global: it assigns truth values to all propositional
variables in the language, even when a particular formula uses only finitely many
of them.
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:truth-assignment-propositional-logic]{Truth Assignment}
\end{itemize}
\end{dependencies}
% END GENERATED ARTIFACT: def:truth-assignment-domain-propositional-logic

\subsection*{Extension of a Truth Assignment}
\label{subsec:extension-of-truth-assignment-propositional-logic}

% BEGIN GENERATED ARTIFACT: def:extended-truth-assignment-propositional-logic
\begin{definitionbox}{Definition (Extension of a Truth Assignment to Formulas)}
\begin{definition}[Extension of a Truth Assignment to Formulas]
\label{def:extended-truth-assignment-propositional-logic}
Let
\[
v:\Prop\to\mathbb{B}
\]
be a truth assignment. The \emph{extension} of \(v\) to well-formed formulas is
the function
\[
\widehat v:\WFF\to\mathbb{B}
\]
defined recursively as follows:
\[
\widehat v(P)=v(P)
\quad\text{for every }P\in\Prop,
\]
\[
\widehat v(\neg\varphi)=\True
\Longleftrightarrow
\widehat v(\varphi)=\False,
\]
\[
\widehat v(\varphi\land\psi)=\True
\Longleftrightarrow
\widehat v(\varphi)=\True
\text{ and }
\widehat v(\psi)=\True,
\]
\[
\widehat v(\varphi\lor\psi)=\True
\Longleftrightarrow
\widehat v(\varphi)=\True
\text{ or }
\widehat v(\psi)=\True,
\]
\[
\widehat v(\varphi\to\psi)=\False
\Longleftrightarrow
\widehat v(\varphi)=\True
\text{ and }
\widehat v(\psi)=\False,
\]
and
\[
\widehat v(\varphi\leftrightarrow\psi)=\True
\Longleftrightarrow
\widehat v(\varphi)=\widehat v(\psi).
\]
\end{definition}
\LeanFormalizes{def:extended-truth-assignment-propositional-logic}{lra-lean}{LRA.VolumeI.Logic.Syntax.Propositional.Formula}{evaluate}{pending}
\LeanFormalizes{def:extended-truth-assignment-propositional-logic}{lra-lean}{LRA.VolumeI.Logic.Syntax.Propositional.Formula}{Formula.and_evaluatesToConjunction}{pending}
\LeanFormalizes{def:extended-truth-assignment-propositional-logic}{lra-lean}{LRA.VolumeI.Logic.Syntax.Propositional.Formula}{Formula.or_evaluatesToDisjunction}{pending}
\LeanFormalizes{def:extended-truth-assignment-propositional-logic}{lra-lean}{LRA.VolumeI.Logic.Syntax.Propositional.Formula}{Formula.iff_evaluatesToBiconditional}{pending}
\end{definitionbox}

\begin{remark*}[Standard quantified statement]
For every truth assignment \(v:\Prop\to\mathbb{B}\), its extension is a function
\[
\widehat v:\WFF\to\mathbb{B}
\]
satisfying the recursive truth clauses for propositional variables, negation,
conjunction, disjunction, material implication, and biconditional.
\end{remark*}

\begin{remark*}[Predicate reading]
\[
\widehat v\in\mathsf{ExtendedTruthAssignment}(v)
\]
means that \(\widehat v\) is the recursively defined truth-value function on
\(\WFF\) induced by the atomic truth assignment \(v\).
\end{remark*}

\begin{remark*}[Interpretation]
The original assignment \(v\) gives truth values only to propositional variables.
The extension \(\widehat v\) evaluates every formula by following the formula's
syntactic construction.
\end{remark*}

\begin{remark*}[Examples]
\[
\begin{array}{c|c}
\varphi & \neg\varphi\\
\hline
\True & \False\\
\False & \True
\end{array}
\qquad
\begin{array}{c|c|c}
\varphi & \psi & \varphi\land\psi\\
\hline
\True & \True & \True\\
\True & \False & \False\\
\False & \True & \False\\
\False & \False & \False
\end{array}
\]
\[
\begin{array}{c|c|c}
\varphi & \psi & \varphi\lor\psi\\
\hline
\True & \True & \True\\
\True & \False & \True\\
\False & \True & \True\\
\False & \False & \False
\end{array}
\qquad
\begin{array}{c|c|c}
\varphi & \psi & \varphi\to\psi\\
\hline
\True & \True & \True\\
\True & \False & \False\\
\False & \True & \True\\
\False & \False & \True
\end{array}
\]
\[
\begin{array}{c|c|c}
\varphi & \psi & \varphi\leftrightarrow\psi\\
\hline
\True & \True & \True\\
\True & \False & \False\\
\False & \True & \False\\
\False & \False & \True
\end{array}
\]
\end{remark*}

\begin{remark*}[Examples]
The truth tables are not additional formation rules. They specify how each
primitive connective acts on truth values once the formula has already been
formed syntactically.
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:truth-assignment-propositional-logic]{Truth Assignment}
  \item \hyperref[def:well-formed-formula]{Well-Formed Formula}
  \item \hyperref[thm:structural-recursion-propositional-formulas]{Structural Recursion for Propositional Formulas}
  \item \hyperref[thm:unique-decomposition-propositional-formulas]{Unique Decomposition for Propositional Formulas}
\end{itemize}
\end{dependencies}
% END GENERATED ARTIFACT: def:extended-truth-assignment-propositional-logic


% BEGIN GENERATED ARTIFACT: thm:unique-extension-truth-assignment-propositional-logic
\begin{theorembox}{Theorem (Unique Extension of a Truth Assignment)}
\begin{theorem}[Unique Extension of a Truth Assignment]
\label{thm:unique-extension-truth-assignment-propositional-logic}
Every truth assignment
\[
v:\Prop\to\mathbb{B}
\]
has a unique extension
\[
\widehat v:\WFF\to\mathbb{B}
\]
satisfying the recursive truth clauses for propositional variables and the
primitive connectives.

\noindent\hyperref[prf:unique-extension-truth-assignment-propositional-logic]{Go to proof.}
\end{theorem}
\end{theorembox}

\begin{remark*}[Standard quantified statement]
\[
(\forall v:\Prop\to\mathbb{B})(\exists!\widehat v:\WFF\to\mathbb{B})
\]
such that \(\widehat v\) agrees with \(v\) on \(\Prop\) and satisfies the
recursive truth clauses for
\[
\neg,\land,\lor,\to,\leftrightarrow.
\]
\end{remark*}

\begin{remark*}[Predicate reading]
\[
v\in\mathsf{TruthAssignment}
\Longrightarrow
\exists!\widehat v\,
\widehat v\in\mathsf{ExtendedTruthAssignment}(v).
\]
\end{remark*}

\begin{remark*}[Interpretation]
This theorem guarantees that semantic evaluation is well-defined. Once the
truth values of the propositional variables are fixed, every well-formed formula
has exactly one truth value.
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:truth-assignment-propositional-logic]{Truth Assignment}
  \item \hyperref[def:extended-truth-assignment-propositional-logic]{Extension of a Truth Assignment to Formulas}
  \item \hyperref[thm:structural-recursion-propositional-formulas]{Structural Recursion for Propositional Formulas}
  \item \hyperref[thm:unique-decomposition-propositional-formulas]{Unique Decomposition for Propositional Formulas}
\end{itemize}
\end{dependencies}
% END GENERATED ARTIFACT: thm:unique-extension-truth-assignment-propositional-logic
% BEGIN GENERATED ARTIFACT: def:satisfaction-propositional-formula
\begin{definitionbox}{Definition (Satisfaction of a Formula)}
\begin{definition}[Satisfaction of a Formula]
\label{def:satisfaction-propositional-formula}
Let \(v\) be a truth assignment and let \(\varphi\in\WFF\). We say that \(v\) satisfies \(\varphi\), written
\[
v\models\varphi,
\]
if
\[
\widehat v(\varphi)=\mathrm{True}.
\]
\end{definition}
\LeanFormalizes{def:satisfaction-propositional-formula}{lra-lean}{LRA.VolumeI.Logic.Model.Propositional.PropositionalModel}{PropositionalModel.satisfies}{pending}
\end{definitionbox}

\begin{remark*}[Standard quantified statement]
\[
v\models\varphi
\Longleftrightarrow
v\in\mathsf{TruthAssignment}
\land
\varphi\in\WFF
\land
\widehat v(\varphi)=\mathrm{True}.
\]
\end{remark*}

\begin{remark*}[Predicate reading]
\[
\operatorname{SatisfiesFormula}(v,\varphi)
\]
means \(v\models\varphi\).
\end{remark*}

\begin{remark*}[Negated quantified statement]
\[
v\not\models\varphi
\Longleftrightarrow
\widehat v(\varphi)=\mathrm{False}.
\]
\end{remark*}

\begin{remark*}[Negation predicate reading]
\[
v\not\models\varphi
\Longleftrightarrow
\widehat v(\varphi)=\mathrm{False}.
\]
\end{remark*}

\begin{remark*}[Failure modes]
\begin{description}
  \item[Exposition.] The failure mode records the negation of formula
  satisfaction.
  \item[\(\operatorname{SatisfiesFormula}\) fails.] Satisfaction of the formula
  fails when the extended truth assignment evaluates the formula as false:
  \[
  v\not\models\varphi
  \Longleftrightarrow
  \widehat v(\varphi)=\mathrm{False}.
  \]
  In predicate-reading form this is recorded as
  \[
  \neg\operatorname{SatisfiesFormula}(v,\varphi).
  \]
\end{description}
\end{remark*}

\begin{remark*}[Interpretation]
The notation \(v\models\varphi\) means that the formula \(\varphi\) is true under the truth assignment \(v\).
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:truth-assignment-propositional-logic]{Truth Assignment}
  \item \hyperref[thm:unique-extension-truth-assignment-propositional-logic]{Unique Extension of a Truth Assignment}
  \item \hyperref[def:well-formed-formula]{Well-Formed Formula}
\end{itemize}
\end{dependencies}
% END GENERATED ARTIFACT: def:satisfaction-propositional-formula
% BEGIN GENERATED ARTIFACT: def:satisfaction-set-propositional-logic
\begin{definitionbox}{Definition (Satisfaction of a Set of Formulas)}
\begin{definition}[Satisfaction of a Set of Formulas]
\label{def:satisfaction-set-propositional-logic}
Let \(\Gamma\subseteq\WFF\). A truth assignment \(v\) satisfies \(\Gamma\), written
\[
v\models\Gamma,
\]
if \(v\models\gamma\) for every \(\gamma\in\Gamma\).
\end{definition}
\LeanFormalizes{def:satisfaction-set-propositional-logic}{lra-lean}{LRA.VolumeI.Logic.Model.Propositional.ModelsAndTheories}{ModelsOfTheory}{pending}
\end{definitionbox}

\begin{remark*}[Standard quantified statement]
\[
v\models\Gamma
\Longleftrightarrow
v\in\mathsf{TruthAssignment}
\land
\Gamma\subseteq\WFF
\land
(\forall\gamma\in\Gamma)(v\models\gamma).
\]
\end{remark*}

\begin{remark*}[Predicate reading]
\[
\operatorname{SatisfiesSet}(v,\Gamma)
\]
means \(v\) satisfies every formula in \(\Gamma\).
\end{remark*}

\begin{remark*}[Negated quantified statement]
\[
v\not\models\Gamma
\Longleftrightarrow
\exists\gamma\in\Gamma\text{ such that }v\not\models\gamma.
\]
\end{remark*}

\begin{remark*}[Negation predicate reading]
\[
v\not\models\Gamma
\Longleftrightarrow
\exists\gamma\in\Gamma\text{ such that }v\not\models\gamma.
\]
\end{remark*}

\begin{remark*}[Failure modes]
\begin{description}
  \item[Exposition.] The failure mode records the negation of simultaneous
  satisfaction of a set.
  \item[\(\operatorname{SatisfiesSet}\) fails.] Satisfaction of the set fails
  when at least one formula in the set is not satisfied:
  \[
  v\not\models\Gamma
  \Longleftrightarrow
  \exists\gamma\in\Gamma\text{ such that }v\not\models\gamma.
  \]
  In predicate-reading form this is recorded as
  \[
  \neg\operatorname{SatisfiesSet}(v,\Gamma).
  \]
\end{description}
\end{remark*}

\begin{remark*}[Interpretation]
Satisfaction of a set means simultaneous satisfaction of every formula in the set.
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:satisfaction-propositional-formula]{Satisfaction of a Formula}
  \item \hyperref[def:truth-assignment-propositional-logic]{Truth Assignment}
\end{itemize}
\end{dependencies}
% END GENERATED ARTIFACT: def:satisfaction-set-propositional-logic
% BEGIN GENERATED ARTIFACT: def:truth-table-propositional-logic
\begin{definitionbox}{Definition (Truth Table)}
\begin{definition}[Truth Table]
\label{def:truth-table-propositional-logic}
Let \(\varphi\in\WFF\) and suppose the variables occurring in \(\varphi\) are among \(P_1,\ldots,P_n\). A truth table for \(\varphi\) is a finite table with one row for each assignment
\[
a:\{P_1,\ldots,P_n\}\to\mathbb{B}
\]
and a final column recording the value of \(\widehat v(\varphi)\) for any truth assignment \(v\) extending \(a\).
\end{definition}
\end{definitionbox}

\begin{remark*}[Standard quantified statement]
\[
\mathsf{TruthTable}(\varphi)
\Longleftrightarrow
\varphi\in\WFF
\land
\operatorname{Var}(\varphi)=\{P_1,\ldots,P_n\}
\land
\text{the table has }2^n\text{ assignment rows}.
\]
\end{remark*}

\begin{remark*}[Interpretation]
A truth table is finite because every formula contains only finitely many variables. It records the complete truth behavior of a formula.
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:formula-variable-set-propositional-logic]{Variable Set of a Formula}
  \item \hyperref[lem:finiteness-of-variables-propositional-formulas]{Finiteness of Variables in a Formula}
  \item \hyperref[thm:unique-extension-truth-assignment-propositional-logic]{Unique Extension of a Truth Assignment}
\end{itemize}
\end{dependencies}
% END GENERATED ARTIFACT: def:truth-table-propositional-logic
% BEGIN GENERATED ARTIFACT: def:tautology-propositional-logic
\begin{definitionbox}{Definition (Tautology)}
\begin{definition}[Tautology]
\label{def:tautology-propositional-logic}
A formula \(\varphi\in\WFF\) is a tautology, written
\[
\models\varphi,
\]
if every truth assignment satisfies \(\varphi\).
\end{definition}
\LeanFormalizes{def:tautology-propositional-logic}{lra-lean}{LRA.VolumeI.Logic.Model.Propositional.ModelsAndTheories}{IsValid}{pending}
\end{definitionbox}

\begin{remark*}[Standard quantified statement]
\[
\models\varphi
\Longleftrightarrow
\varphi\in\WFF
\land
(\forall v:\Prop\to\mathbb{B})(v\models\varphi).
\]
\end{remark*}

\begin{remark*}[Predicate reading]
\[
\operatorname{IsTautology}(\varphi)
\]
means \(\models\varphi\).
\end{remark*}

\begin{remark*}[Interpretation]
A tautology is true under every possible assignment of truth values to propositional variables.
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:satisfaction-propositional-formula]{Satisfaction of a Formula}
  \item \hyperref[def:truth-assignment-propositional-logic]{Truth Assignment}
\end{itemize}
\end{dependencies}
% END GENERATED ARTIFACT: def:tautology-propositional-logic
% BEGIN GENERATED ARTIFACT: def:contradiction-propositional-logic
\begin{definitionbox}{Definition (Contradiction)}
\begin{definition}[Contradiction]
\label{def:contradiction-propositional-logic}
A formula \(\varphi\in\WFF\) is a contradiction if no truth assignment satisfies \(\varphi\).
\end{definition}
\end{definitionbox}

\begin{remark*}[Standard quantified statement]
\[
\operatorname{Contradiction}(\varphi)
\Longleftrightarrow
\varphi\in\WFF
\land
(\forall v:\Prop\to\mathbb{B})(v\not\models\varphi).
\]
\end{remark*}

\begin{remark*}[Predicate reading]
\[
\operatorname{Contradiction}(\varphi)
\]
means that \(\varphi\) is false under every truth assignment.
\end{remark*}

\begin{remark*}[Interpretation]
A contradiction has no true row in its truth table.
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:satisfaction-propositional-formula]{Satisfaction of a Formula}
  \item \hyperref[def:truth-assignment-propositional-logic]{Truth Assignment}
\end{itemize}
\end{dependencies}
% END GENERATED ARTIFACT: def:contradiction-propositional-logic
% BEGIN GENERATED ARTIFACT: def:satisfiable-formula-propositional-logic
\begin{definitionbox}{Definition (Satisfiable Formula)}
\begin{definition}[Satisfiable Formula]
\label{def:satisfiable-formula-propositional-logic}
A formula \(\varphi\in\WFF\) is satisfiable if at least one truth assignment satisfies it.
\end{definition}
\end{definitionbox}

\begin{remark*}[Standard quantified statement]
\[
\operatorname{Satisfiable}(\varphi)
\Longleftrightarrow
\varphi\in\WFF
\land
\exists v:\Prop\to\mathbb{B}\text{ such that }v\models\varphi.
\]
\end{remark*}

\begin{remark*}[Predicate reading]
\[
\operatorname{Satisfiable}(\varphi)
\]
means that \(\varphi\) is true under at least one truth assignment.
\end{remark*}

\begin{remark*}[Interpretation]
A satisfiable formula has at least one true row in its truth table.
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:satisfaction-propositional-formula]{Satisfaction of a Formula}
  \item \hyperref[def:truth-assignment-propositional-logic]{Truth Assignment}
\end{itemize}
\end{dependencies}
% END GENERATED ARTIFACT: def:satisfiable-formula-propositional-logic
% BEGIN GENERATED ARTIFACT: def:contingency-propositional-logic
\begin{definitionbox}{Definition (Contingency)}
\begin{definition}[Contingency]
\label{def:contingency-propositional-logic}
A formula \(\varphi\in\WFF\) is a contingency if it is satisfiable but not a tautology.
\end{definition}
\end{definitionbox}

\begin{remark*}[Standard quantified statement]
\[
\operatorname{Contingency}(\varphi)
\Longleftrightarrow
\operatorname{Satisfiable}(\varphi)
\land
\neg \operatorname{IsTautology}(\varphi).
\]
\end{remark*}

\begin{remark*}[Predicate reading]
\[
\operatorname{Contingency}(\varphi)
\]
means that \(\varphi\) is true under some truth assignments and false under others.
\end{remark*}

\begin{remark*}[Interpretation]
A contingency has at least one true row and at least one false row in its truth table.
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:satisfiable-formula-propositional-logic]{Satisfiable Formula}
  \item \hyperref[def:tautology-propositional-logic]{Tautology}
\end{itemize}
\end{dependencies}
% END GENERATED ARTIFACT: def:contingency-propositional-logic
% BEGIN GENERATED ARTIFACT: def:logical-consequence-propositional-logic
\begin{definitionbox}{Definition (Logical Consequence)}
\begin{definition}[Logical Consequence]
\label{def:logical-consequence-propositional-logic}
Let \(\Gamma\subseteq\WFF\) and \(\varphi\in\WFF\). The formula \(\varphi\) is a logical consequence of \(\Gamma\), written
\[
\Gamma\models\varphi,
\]
if every truth assignment satisfying \(\Gamma\) also satisfies \(\varphi\).
\end{definition}
\LeanFormalizes{def:logical-consequence-propositional-logic}{lra-lean}{LRA.VolumeI.Logic.Model.Propositional.ModelsAndTheories}{SemanticConsequence}{pending}
\end{definitionbox}

\begin{remark*}[Standard quantified statement]
\[
\Gamma\models\varphi
\Longleftrightarrow
\Gamma\subseteq\WFF
\land
\varphi\in\WFF
\land
(\forall v:\Prop\to\mathbb{B})
\left[
v\models\Gamma\to v\models\varphi
\right].
\]
\end{remark*}

\begin{remark*}[Predicate reading]
\[
\operatorname{LogicalConsequence}(\Gamma,\varphi)
\]
means \(\Gamma\models\varphi\).
\end{remark*}

\begin{remark*}[Negated quantified statement]
\[
\Gamma\not\models\varphi
\Longleftrightarrow
\exists v:\Prop\to\mathbb{B}
\left[
v\models\Gamma
\land
v\not\models\varphi
\right].
\]
\end{remark*}

\begin{remark*}[Negation predicate reading]
\[
\Gamma\not\models\varphi
\Longleftrightarrow
\exists v:\Prop\to\mathbb{B}
\left[
v\models\Gamma
\land
v\not\models\varphi
\right].
\]
\end{remark*}

\begin{remark*}[Failure modes]
\begin{description}
  \item[Exposition.] The failure mode records the countermodel form of failed
  logical consequence.
  \item[\(\operatorname{LogicalConsequence}\) fails.] Logical consequence
  fails when there is a countermodel assignment satisfying every premise but
  not the conclusion:
  \[
  \Gamma\not\models\varphi
  \Longleftrightarrow
  \exists v:\Prop\to\mathbb{B}
  \left[
  v\models\Gamma
  \land
  v\not\models\varphi
  \right].
  \]
  In predicate-reading form this is recorded as
  \[
  \neg\operatorname{LogicalConsequence}(\Gamma,\varphi).
  \]
\end{description}
\end{remark*}

\begin{remark*}[Interpretation]
Logical consequence means truth preservation from premises to conclusion. There is no truth assignment under which all premises are true and the conclusion is false.
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:satisfaction-set-propositional-logic]{Satisfaction of a Set of Formulas}
  \item \hyperref[def:satisfaction-propositional-formula]{Satisfaction of a Formula}
\end{itemize}
\end{dependencies}
% END GENERATED ARTIFACT: def:logical-consequence-propositional-logic
% BEGIN GENERATED ARTIFACT: def:logical-equivalence-propositional-logic
\begin{definitionbox}{Definition (Logical Equivalence)}
\begin{definition}[Logical Equivalence]
\label{def:logical-equivalence-propositional-logic}
Formulas \(\varphi,\psi\in\WFF\) are logically equivalent, written
\[
\varphi\equiv\psi,
\]
if they have the same truth value under every truth assignment.
\end{definition}
\end{definitionbox}

\begin{remark*}[Standard quantified statement]
\[
\varphi\equiv\psi
\Longleftrightarrow
\varphi,\psi\in\WFF
\land
(\forall v:\Prop\to\mathbb{B})
\left[
\widehat v(\varphi)=\widehat v(\psi)
\right].
\]
\end{remark*}

\begin{remark*}[Predicate reading]
\[
\operatorname{LogicalEquivalence}(\varphi,\psi)
\]
means \(\varphi\equiv\psi\).
\end{remark*}

\begin{remark*}[Interpretation]
Logically equivalent formulas have identical truth-table columns. They are interchangeable for semantic purposes.
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[thm:unique-extension-truth-assignment-propositional-logic]{Unique Extension of a Truth Assignment}
  \item \hyperref[def:truth-assignment-propositional-logic]{Truth Assignment}
\end{itemize}
\end{dependencies}
% END GENERATED ARTIFACT: def:logical-equivalence-propositional-logic
% BEGIN GENERATED ARTIFACT: lem:coincidence-lemma-propositional-logic
\begin{lemmabox}{Lemma (Coincidence Lemma for Propositional Logic)}
\begin{lemma}[Coincidence Lemma for Propositional Logic]
\label{lem:coincidence-lemma-propositional-logic}
Let \(v,w:\Prop\to\mathbb{B}\) be truth assignments. If \(v\) and \(w\) agree on every propositional variable occurring in \(\varphi\), then they assign the same truth value to \(\varphi\):
\[
v|_{\operatorname{Var}(\varphi)}
=
w|_{\operatorname{Var}(\varphi)}
\Longrightarrow
\widehat v(\varphi)=\widehat w(\varphi).
\]

\noindent\hyperref[prf:coincidence-lemma-propositional-logic]{Go to proof.}
\end{lemma}
\end{lemmabox}
\begin{remark*}[Standard quantified statement]
\[
(\forall\varphi\in\WFF)
(\forall v,w:\Prop\to\mathbb{B})
\left[
(\forall P\in\operatorname{Var}(\varphi))(v(P)=w(P))
\to
\widehat v(\varphi)=\widehat w(\varphi)
\right].
\]
\end{remark*}

\begin{remark*}[Predicate reading]
\[
(\forall\varphi\in\WFF)
(\forall v,w:\Prop\to\mathbb{B})
\left[
(\forall P\in\operatorname{Var}(\varphi))(v(P)=w(P))
\to
\widehat v(\varphi)=\widehat w(\varphi)
\right].
\]
\end{remark*}




\begin{remark*}[Interpretation]
The truth value of a formula depends only on the variables actually occurring in that formula.
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:formula-variable-set-propositional-logic]{Variable Set of a Formula}
  \item \hyperref[thm:unique-extension-truth-assignment-propositional-logic]{Unique Extension of a Truth Assignment}
  \item \hyperref[thm:structural-induction-propositional-formulas]{Structural Induction for Propositional Formulas}
\end{itemize}
\end{dependencies}
% END GENERATED ARTIFACT: lem:coincidence-lemma-propositional-logic
% BEGIN GENERATED ARTIFACT: thm:finite-truth-table-propositional-logic
\begin{theorembox}{Theorem (Finite Truth-Table Theorem)}
\begin{theorem}[Finite Truth-Table Theorem]
\label{thm:finite-truth-table-propositional-logic}
Every propositional formula has a finite truth table. More precisely, if \(\operatorname{Var}(\varphi)\) has \(n\) elements, then \(\varphi\) has a truth table with \(2^n\) rows.

\noindent\hyperref[prf:finite-truth-table-propositional-logic]{Go to proof.}
\end{theorem}
\end{theorembox}

\begin{remark*}[Standard quantified statement]
\[
(\forall\varphi\in\WFF)
\left[
|\operatorname{Var}(\varphi)|=n
\to
\mathsf{TruthTable}(\varphi)\text{ has }2^n\text{ rows}
\right].
\]
\end{remark*}

\begin{remark*}[Interpretation]
Truth tables are finite because every formula contains only finitely many propositional variables.
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:truth-table-propositional-logic]{Truth Table}
  \item \hyperref[lem:finiteness-of-variables-propositional-formulas]{Finiteness of Variables in a Formula}
\end{itemize}
\end{dependencies}
% END GENERATED ARTIFACT: thm:finite-truth-table-propositional-logic
% BEGIN GENERATED ARTIFACT: def:unary-boolean-function-propositional-logic
\begin{definitionbox}{Definition (Unary Boolean Function)}
\begin{definition}[Unary Boolean Function]
\label{def:unary-boolean-function-propositional-logic}
A unary Boolean function is a function
\[
f:\mathbb{B}\to\mathbb{B}.
\]
\end{definition}
\end{definitionbox}
\begin{remark*}[Standard quantified statement]
For every admissible object satisfying the hypotheses of the statement, the
assertion holds in the formal language under discussion.
\end{remark*}


\begin{remark*}[Predicate reading]
\[
UnaryBooleanFunction(f)
\]
means that \(f\) takes one truth value as input and returns one truth value.
\end{remark*}

\begin{remark*}[Interpretation]
A unary Boolean function has one input row coordinate. It is the semantic shape of a one-place truth operation such as negation.
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:truth-value-propositional-logic]{Truth Value}
\end{itemize}
\end{dependencies}
% END GENERATED ARTIFACT: def:unary-boolean-function-propositional-logic
% BEGIN GENERATED ARTIFACT: def:binary-boolean-function-propositional-logic
\begin{definitionbox}{Definition (Binary Boolean Function)}
\begin{definition}[Binary Boolean Function]
\label{def:binary-boolean-function-propositional-logic}
A binary Boolean function is a function
\[
f:\mathbb{B}^2\to\mathbb{B},
\]
equivalently a function
\[
f:\mathbb{B}\times\mathbb{B}\to\mathbb{B}.
\]
\end{definition}
\end{definitionbox}
\begin{remark*}[Standard quantified statement]
For every admissible object satisfying the hypotheses of the statement, the
assertion holds in the formal language under discussion.
\end{remark*}


\begin{remark*}[Predicate reading]
\[
BinaryBooleanFunction(f)
\]
means that \(f\) takes an ordered pair of truth values as input and returns one truth value.
\end{remark*}

\begin{remark*}[Interpretation]
A binary Boolean function assigns one output truth value to each of the four input rows
\[
(T,T),\quad (T,F),\quad (F,T),\quad (F,F).
\]
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:truth-value-propositional-logic]{Truth Value}
  \item \hyperref[def:unary-boolean-function-propositional-logic]{Unary Boolean Function}
\end{itemize}
\end{dependencies}
% END GENERATED ARTIFACT: def:binary-boolean-function-propositional-logic
% BEGIN GENERATED ARTIFACT: def:boolean-function-propositional-logic
\begin{definitionbox}{Definition (Boolean Function)}
\begin{definition}[Boolean Function]
\label{def:boolean-function-propositional-logic}
An \(n\)-ary Boolean function is a function
\[
f:\mathbb{B}^n\to\mathbb{B}.
\]
\end{definition}
\end{definitionbox}

\begin{remark*}[Standard quantified statement]
\[
BooleanFunction_n(f)
\Longleftrightarrow
f:\mathbb{B}^n\to\mathbb{B}.
\]
\end{remark*}

\begin{remark*}[Predicate reading]
\[
BooleanFunction_n(f)
\]
means that \(f\) takes \(n\) truth values as input and returns one truth value.
\end{remark*}

\begin{remark*}[Interpretation]
Boolean functions are the abstract truth-functions that formulas may represent.
\end{remark*}

\begin{remark*}[Exposition]
An input to an \(n\)-ary Boolean function is a tuple
\((b_1,\ldots,b_n)\in\mathbb{B}^n\). These tuples are the rows of the truth table, and there are \(2^n\) of them. The Boolean function is not a single row; it is the full rule assigning exactly one truth value in \(\mathbb{B}\) to every row.
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:truth-value-propositional-logic]{Truth Value}
  \item \hyperref[def:unary-boolean-function-propositional-logic]{Unary Boolean Function}
  \item \hyperref[def:binary-boolean-function-propositional-logic]{Binary Boolean Function}
\end{itemize}
\end{dependencies}
% END GENERATED ARTIFACT: def:boolean-function-propositional-logic
% BEGIN GENERATED ARTIFACT: def:truth-function-induced-by-formula
\begin{definitionbox}{Definition (Truth Function Induced by a Formula)}
\begin{definition}[Truth Function Induced by a Formula]
\label{def:truth-function-induced-by-formula}
Let \(\varphi\in\WFF\), and suppose \(\operatorname{Var}(\varphi)\subseteq\{P_1,\ldots,P_n\}\). The truth function induced by \(\varphi\) is the Boolean function
\[
f_\varphi:\mathbb{B}^n\to\mathbb{B}
\]
defined by
\[
f_\varphi(b_1,\ldots,b_n)=\widehat v(\varphi),
\]
where \(v(P_i)=b_i\) for each \(i\).
\end{definition}
\end{definitionbox}

\begin{remark*}[Standard quantified statement]
\[
f_\varphi(b_1,\ldots,b_n)=\widehat v(\varphi)
\quad
\text{whenever }
v(P_i)=b_i\text{ for }1\le i\le n.
\]
\end{remark*}

\begin{remark*}[Predicate reading]
\[
\operatorname{TruthFunctionOfFormula}(f_\varphi,\varphi)
\]
means that \(f_\varphi\) records the truth behavior of \(\varphi\).
\end{remark*}

\begin{remark*}[Interpretation]
The induced truth function packages the truth table of a formula as a Boolean function.
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:boolean-function-propositional-logic]{Boolean Function}
  \item \hyperref[def:formula-variable-set-propositional-logic]{Variable Set of a Formula}
  \item \hyperref[thm:unique-extension-truth-assignment-propositional-logic]{Unique Extension of a Truth Assignment}
  \item \hyperref[lem:coincidence-lemma-propositional-logic]{Coincidence Lemma for Propositional Logic}
\end{itemize}
\end{dependencies}
% END GENERATED ARTIFACT: def:truth-function-induced-by-formula
% BEGIN GENERATED ARTIFACT: thm:counting-boolean-functions
\begin{theorembox}{Theorem (Counting Boolean Functions)}
\begin{theorem}[Counting Boolean Functions]
\label{thm:counting-boolean-functions}
There are exactly
\[
2^{2^n}
\]
Boolean functions \(\mathbb{B}^n\to\mathbb{B}\).

\noindent\hyperref[prf:counting-boolean-functions]{Go to proof.}
\end{theorem}
\end{theorembox}

\begin{remark*}[Standard quantified statement]
\[
\left|\{f:\mathbb{B}^n\to\mathbb{B}\}\right|=2^{2^n}.
\]
\end{remark*}

\begin{remark*}[Interpretation]
The domain \(\mathbb{B}^n\) has \(2^n\) elements. A Boolean function chooses one of two truth values for each input row.
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:boolean-function-propositional-logic]{Boolean Function}
\end{itemize}
\end{dependencies}
% END GENERATED ARTIFACT: thm:counting-boolean-functions
