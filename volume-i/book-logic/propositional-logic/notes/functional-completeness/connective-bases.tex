...
\[
  B\in\mathsf{ConnectiveBasis}
\] means that \(B\) is the chosen primitive
...
\begin{remark*}[Predicate reading]
\[
  \operatorname{DefinableFrom}(\circ,B)
\] means that \(\circ\) can be treated as
...
\[
  \operatorname{FunctionallyComplete}(B)
\] means that formulas over \(B\) can
...
\[
  B\equiv_{\mathrm{expr}} C
\] means that the two bases can define exactly the
