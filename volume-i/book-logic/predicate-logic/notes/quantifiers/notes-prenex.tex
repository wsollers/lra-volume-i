...
\[
  \Pi\in\mathsf{QuantifierPrefix}
\] means that \(\Pi\) is a finite string
...
\begin{remark*}[Predicate reading]
\[
  \psi\in\mathsf{Matrix}(\Phi)
\] means that \(\psi\) is the matrix of the
formula \(\Phi\).
\end{remark*}
...
\begin{remark*}[Predicate reading]
\[
  \Phi\in\mathsf{PrenexFormula}
\] means that all quantifiers of \(\Phi\)
...
\begin{description}
  \item[Exposition.] The failure mode records the negation of the prenex-form
  predicate.
  \item[\(\mathsf{PrenexFormula}\) fails.] The prenex predicate fails when the
  formula cannot be written as an initial quantifier prefix followed by a
...
\begin{remark*}[Predicate reading]
\[
  \psi\in\mathsf{PrenexNormalForm}(\varphi)
\] means that \(\psi\) is a
