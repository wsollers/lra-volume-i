...
\[
  \operatorname{StrongerFormula}(A,B)
\] means that \(A\) entails \(B\), but
\(B\) does not entail \(A\).
\end{remark*}
...
\begin{description}
  ...
  stronger-formula predicate.
  \item[\(\operatorname{StrongerFormula}(A,B)\) entailment clause fails.] The
  stronger-formula predicate fails if \(A\) does not entail \(B\):
  \[
  A\not\models B.
  \]
  In predicate-reading form this contributes to
  \[
  \neg\operatorname{StrongerFormula}(A,B).
  \]
  \item[\(\operatorname{StrongerFormula}(A,B)\) strictness clause fails.] The
  stronger-formula predicate also fails if \(B\) entails \(A\), so \(A\) is not
...
\begin{remark*}[Predicate reading]
\[
  \operatorname{WeakerFormula}(B,A)
\] means that \(B\) is entailed by \(A\) but
does not entail \(A\).
\end{remark*}
...
\begin{remark*}[Predicate reading]
\[
  \operatorname{QuantifierOrder}(\Phi,(Q_1,\ldots,Q_n))
\] records the order of
