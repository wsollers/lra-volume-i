...
\]
\end{definition}
\LeanFormalizes{def:fol-satisfaction}{lra-lean}{LRA.VolumeI.Logic.Semantics.Satisfaction}{Satisfies}{pending}
\end{definitionbox}
...
\[
  \operatorname{Satisfaction}(M,s,\varphi)
\] means
...
\[
  \neg\operatorname{Satisfaction}(M,s,\varphi)
\] means that \(\varphi\) is not
satisfied in \(M\) under \(s\).
\end{remark*}

\begin{remark*}[Failure modes]
\begin{description}
  \item[Exposition.] Satisfaction fails according to the outermost formation rule of the formula:
...
\item[\(\operatorname{Satisfies}\) fails.] \[
...
\(\mathcal M\).
\end{definition}
\LeanFormalizes{def:truth-structure}{lra-lean}{LRA.VolumeI.Logic.Semantics.Sentence}{satisfies_sentence_iff}{pending}
\end{definitionbox}
...
\begin{remark*}[Predicate reading]
\[
  \operatorname{TruthInStructure}(M,\varphi)
\] means
\(\mathcal M\models\varphi\).
\end{remark*}
...
\begin{remark*}[Negation predicate reading]
\[
  \neg\operatorname{TruthInStructure}(M,\varphi)
\] means that \(\varphi\) is
false in \(M\).
\end{remark*}

\begin{remark*}[Failure modes]
\begin{description}
  \item[Exposition.] Truth in a structure fails when some assignment makes the sentence fail. For
...
\begin{description}
  \item[Exposition.] The named branch below records the displayed failure mode.
\item[\(\operatorname{TruthInStructure}\) fails.] \[
...
\begin{remark*}[Predicate reading]
\[
  \operatorname{Valid}(\varphi)
\] means that \(\varphi\) is satisfied in
...
\begin{remark*}[Negation predicate reading]
\[
  \neg\operatorname{Valid}(\varphi)
\] means that some structure and
assignment make \(\varphi\) fail.
\end{remark*}

\begin{remark*}[Failure modes]
\begin{description}
  \item[Exposition.] Validity fails by a counterexample structure together with an assignment.
\end{description}
\end{remark*}
...
\begin{remark*}[Negation predicate reading]
\[
  \neg\operatorname{Satisfiable}(\varphi)
\] means that no structure and
assignment satisfy \(\varphi\).
\end{remark*}

\begin{remark*}[Failure modes]
\begin{description}
  \item[Exposition.] Satisfiability fails only globally: every attempted structure and assignment
...
\begin{description}
  \item[Exposition.] The named branch below records the displayed failure mode.
\item[\(\operatorname{Satisfiable}\) fails.] \[
