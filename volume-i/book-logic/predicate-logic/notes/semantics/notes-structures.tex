...
Hereafter, predicate logic means first-order logic with equality: the
distinguished equality symbol has the identity behavior axiomatized in
\hyperref[ch:axiomatic-equality]{Axiomatic Equality}.
\end{definition}
\LeanFormalizes{def:fo-language}{lra-lean}{LRA.VolumeI.Logic.Language.FirstOrder.Signature}{FirstOrderLanguage}{pending}
\LeanFormalizes{def:fo-language}{lra-lean}{LRA.VolumeI.Logic.Language.FirstOrder.Signature}{Signature}{pending}
\end{definitionbox}
...
\end{aligned}
\]
together with equality, the usual logical connectives, quantifiers, and
punctuation.
\end{remark*}

\begin{remark*}[Predicate reading]
\[
  L\in\mathsf{FirstOrderLanguage}
\] means that \(L\) supplies the variable
...
\[
  L\notin\mathsf{FirstOrderLanguage}
\] means that \(L\) lacks some
required syntactic stock or arity assignment.
\end{remark*}

\begin{remark*}[Failure modes]
\begin{description}
  \item[Exposition.] A proposed language fails when its non-logical symbols are not sorted by role,
...
\end{definition}
\LeanFormalizes{def:structure}{lra-lean}{LRA.VolumeI.Logic.Model.Model}{Model}{pending}
\end{definitionbox}
...
\begin{remark*}[Predicate reading]
\[
  M\in\mathsf{Structure}(L)
\] means that \(M\) is a structure for the
...
\begin{remark*}[Negation predicate reading]
\[
  M\notin\mathsf{Structure}(L)
\] means that \(M\) is not a well-formed
structure for \(L\).
\end{remark*}

\begin{remark*}[Failure modes]
\begin{description}
  \item[Exposition.] A proposed structure fails if the domain is empty, if a symbol is left
...
\begin{remark*}[Predicate reading]
\[
  s\in\mathsf{VariableAssignment}(M)
\] means that \(s\) assigns a domain
...
\begin{remark*}[Negation predicate reading]
\[
  s\notin\mathsf{VariableAssignment}(M)
\] means that \(s\) is not a
function assigning each variable a domain element of \(M\).
\end{remark*}

\begin{remark*}[Failure modes]
\begin{description}
  \item[Exposition.] An assignment fails when it leaves a variable without a value or assigns a
...
\begin{description}
  \item[Exposition.] The named branch below records the displayed failure mode.
\item[\(\mathsf{VariableAssignment}\) fails.] \[
...
\]
\end{definition}
\LeanFormalizes{def:mod-assign}{lra-lean}{LRA.VolumeI.Logic.Semantics.Assignment}{updateAssignment}{pending}
\end{definitionbox}
...
\begin{remark*}[Predicate reading]
\[
  s'\in\mathsf{ModifiedAssignment}(s,x,d)
\] means that \(s'\) is the
...
\begin{remark*}[Negation predicate reading]
\[
  s'\notin\mathsf{ModifiedAssignment}(s,x,d)
\] means that \(s'\) is not the
one-variable update of \(s\) at \(x\) with value \(d\).
\end{remark*}

\begin{remark*}[Failure modes]
\begin{description}
  \item[Exposition.] A proposed modified assignment fails if it does not change \(x\) to \(d\), or
...
\begin{description}
  \item[Exposition.] The named branch below records the displayed failure mode.
\item[\(\mathsf{ModifiedAssignment}\) fails.] \[
...
\]
\end{definition}
\LeanFormalizes{def:term-interp}{lra-lean}{LRA.VolumeI.Logic.Semantics.TermEvaluation}{evaluateTerm}{pending}
\end{definitionbox}
...
\begin{remark*}[Predicate reading]
\[
  a\in\mathsf{TermEvaluation}(M,s,t)
\] means that the term \(t\) evaluates
...
\begin{remark*}[Negation predicate reading]
\[
  a\notin\mathsf{TermEvaluation}(M,s,t)
\] means that \(a\) is not the value
of \(t\) in \(M\) under \(s\).
\end{remark*}

\begin{remark*}[Failure modes]
\begin{description}
  \item[Exposition.] Term evaluation fails to have the proposed value when a variable, constant, or
...
\begin{description}
  \item[Exposition.] The named branch below records the displayed failure mode.
\item[\(\mathsf{TermEvaluation}\) fails.] \[
