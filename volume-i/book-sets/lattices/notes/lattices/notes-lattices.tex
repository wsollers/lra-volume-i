...
\begin{definitionbox}{Definition (Lattice)}
...
\[
  \mathsf{Lattice}(L,\leq,\vee,\wedge)
\] means that every pair of elements of
$L$ has a join and a meet in the poset $(L,\leq)$.
\end{remark*}

\begin{remark*}[Interpretation]
A lattice is a poset in which binary suprema and binary infima always exist.
\end{remark*}
...
the divisibility discussion in the supremum section illustrates.
\end{remark*}
...
\begin{definitionbox}{Definition (Complete Lattice)}
...
\begin{remark*}[Predicate reading]
\[
  \mathsf{CompleteLattice}(L,\leq,\vee,\wedge)
\] means every subset of $L$ has a
supremum and an infimum in $L$.
\end{remark*}

\begin{remark*}[Interpretation]
A complete lattice has all joins and meets, not just binary ones.
\end{remark*}
...
Concretely: in $(\mathcal{P}(X), \subseteq)$, $\sup \varnothing =
...
\begin{definitionbox}{Definition (Distributive Lattice)}
\begin{definition}[Distributive Lattice]\label{def:distributive-lattice}
...
\begin{remark*}[Standard quantified statement]
$\mathsf{DistributiveLattice}(L,\leq,\vee,\wedge)\leftrightarrow
\mathsf{Lattice}(L,\leq,\vee,\wedge)\wedge
\forall a,b,c\in L,\; a\wedge(b\vee c)=(a\wedge b)\vee(a\wedge c)$.
\end{remark*}

\begin{remark*}[Predicate reading]
\[
  \mathsf{DistributiveLattice}(L,\leq,\vee,\wedge)
\] means meet distributes over
join in the lattice operations.
\end{remark*}

\begin{remark*}[Interpretation]
Distributivity is the lattice version of the familiar set-algebra law for
...
\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:lattice]{Lattice}
  \item \hyperref[def:subset]{Subset}
  \item \hyperref[def:sup-inf]{Supremum and Infimum}
\end{itemize}
\end{dependencies}

\begin{definitionbox}{Definition (Complement and Boolean Lattice)}
\begin{definition}[Complement and Boolean Lattice]\label{def:boolean-lattice}
...
\begin{remark*}[Standard quantified statement]
$\mathsf{BooleanLattice}(L,\leq,\vee,\wedge,{}',\bot,\top)$ means
$(L,\leq)$ is a bounded distributive lattice and
$\forall a\in L,\;\exists a'\in L,\;a\vee a'=\top\wedge a\wedge a'=\bot$.
\end{remark*}

\begin{remark*}[Predicate reading]
\[
  \mathsf{BooleanLattice}(L,\leq,\vee,\wedge,{}',\bot,\top)
\] means every element
has a complement in a bounded distributive lattice.
\end{remark*}

\begin{remark*}[Interpretation]
A Boolean lattice abstracts the algebra of sets with union, intersection, and
complement.
\end{remark*}
...
\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:distributive-lattice]{Distributive Lattice}
...
\begin{theorembox}{Theorem (Knaster--Tarski Fixed-Point Theorem)}
\begin{theorem}[Knaster--Tarski Fixed-Point Theorem]\label{thm:knaster-tarski}
...
\]
\hyperref[prf:knaster-tarski]{\textit{Go to proof.}}
\end{theorem}
\end{theorembox}
...
\begin{remark*}[Predicate reading]
\[
  \operatorname{KnasterTarskiFixedPoint}(f,\mathsf{CompleteLattice}(L,\leq,\vee,\wedge))
\]
means that the monotone self-map $f:L\to L$ has least and greatest fixed
points in the complete lattice.
\end{remark*}

\begin{remark*}[Interpretation]
Every monotone self-map of a complete lattice has an extremal range of fixed
points.
\end{remark*}
...
further:
\begin{itemize}
  \item In set theory, it is used to prove the Schr\"{o}der--Bernstein theorem
  ...
  \item In measure theory, it is used to construct the smallest
        $\sigma$-algebra containing a given collection of sets (the
...
The Orderings chapter introduced Hasse diagrams, the duality principle, and
the connection between duality and the passage from $\sup$ to $\inf$.
\end{remark*}
...
\begin{itemize}
  \item \hyperref[def:distributive-lattice]{Distributive Lattice}
  \item \hyperref[def:complete-lattice]{Complete Lattice}
  \item \hyperref[def:boolean-lattice]{Complement and Boolean Lattice}
\end{itemize}
\end{dependencies}
