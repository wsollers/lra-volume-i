...
\end{definition}
\LeanFormalizes{def:reflexive}{lra-lean}{LRA.VolumeI.Relations.NamedRelations}{Reflexive}{pending}
\end{definitionbox}
...
Reflexivity says that the relation includes every diagonal pair $(a,a)$ from
$A\times A$. It is a global condition on the whole carrier, not a condition on
only the elements that happen to occur in some related pair.
\end{remark*}
...
\end{definition}
\LeanFormalizes{def:irreflexive}{lra-lean}{LRA.VolumeI.Relations.NamedRelations}{Irreflexive}{pending}
\end{definitionbox}
...
\begin{remark*}[Interpretation]
Irreflexivity says that the relation avoids the entire diagonal of
$A\times A$. It is stronger than failing to be reflexive, because it requires
every element to fail self-relation.
\end{remark*}
...
\end{definition}
\LeanFormalizes{def:symmetric}{lra-lean}{LRA.VolumeI.Relations.NamedRelations}{Symmetric}{pending}
\end{definitionbox}
...
\begin{remark*}[Interpretation]
Symmetry says that every directed relation arrow can be reversed. It imposes
no requirement that unrelated pairs become related.
\end{remark*}
...
\end{definition}
\LeanFormalizes{def:antisymmetric}{lra-lean}{LRA.VolumeI.Relations.NamedRelations}{Antisymmetric}{pending}
\end{definitionbox}
...
\begin{remark*}[Interpretation]
Antisymmetry allows two-way relation only when both endpoints are the same
element. It permits self-relation and therefore differs from asymmetry.
\end{remark*}
...
\end{definition}
\LeanFormalizes{def:asymmetric}{lra-lean}{LRA.VolumeI.Relations.NamedRelations}{Asymmetric}{pending}
\end{definitionbox}
...
\[
  ...
  \iff
  \forall a,b\in A,\ ((a,b)\in R\to (b,a)\notin R).
\]
\end{remark*}

\begin{remark*}[Interpretation]
Asymmetry says that relation can occur in at most one direction between any
ordered pair of elements. Substituting $a=b$ also rules out self-relation.
\end{remark*}
...
\end{definition}
\LeanFormalizes{def:transitive}{lra-lean}{LRA.VolumeI.Relations.NamedRelations}{Transitive}{pending}
\end{definitionbox}
...
\begin{remark*}[Interpretation]
Transitivity says that an $R$-step from $a$ to $b$ followed by an $R$-step
from $b$ to $c$ forces a direct $R$-step from $a$ to $c$.
\end{remark*}
...
\]
\end{definition}
\LeanFormalizes{def:total-rel}{lra-lean}{LRA.VolumeI.Relations.NamedRelations}{Total}{pending}
\end{definitionbox}
...
\begin{remark*}[Interpretation]
A total relation makes every pair of elements comparable in at least one
direction. It does not by itself assert reflexivity, antisymmetry, or
transitivity.
\end{remark*}
...
and transitive.
\end{definition}
\LeanFormalizes{def:equivalence-rel}{lra-lean}{LRA.VolumeI.Relations.NamedRelations}{Equivalence}{pending}
\end{definitionbox}
...
\[
  ...
  \iff
  \operatorname{Reflexive}(R,A)
  \wedge \operatorname{Symmetric}(R,A)
  \wedge \operatorname{Transitive}(R,A).
\]
\end{remark*}

\begin{remark*}[Interpretation]
An equivalence relation formalizes sameness relative to a chosen criterion.
Its three component properties make equivalence classes well behaved.
\end{remark*}
...
\end{definition}
\LeanFormalizes{def:preorder}{lra-lean}{LRA.VolumeI.Order.Posets}{Preorder}{pending}
\end{definitionbox}
...
\begin{remark*}[Interpretation]
A preorder has enough structure to compare elements reflexively and chain
comparisons transitively, but it may identify distinct elements as mutually
comparable.
\end{remark*}
...
transitive.
\end{definition}
\LeanFormalizes{def:partial-order}{lra-lean}{LRA.VolumeI.Order.Posets}{PartialOrder}{pending}
\end{definitionbox}
...
\[
  ...
  \iff
  \operatorname{Reflexive}(R,A)
  \wedge \operatorname{Antisymmetric}(R,A)
  \wedge \operatorname{Transitive}(R,A).
\]
\end{remark*}

\begin{remark*}[Interpretation]
A partial order is a preorder in which mutual comparability forces equality.
This removes the identification ambiguity that can occur in preorders.
\end{remark*}
...
\begin{definition}[Strict Partial Order]\label{def:strict-partial-order-relations}
$R$ is a \emph{strict partial order} on $A$ if it is irreflexive and
transitive.
\end{definition}
\LeanFormalizes{def:strict-partial-order-relations}{lra-lean}{LRA.VolumeI.Order.Posets}{StrictOrder}{pending}
\end{definitionbox}

\begin{remark*}[Standard quantified statement]
$R$ is a strict partial order on $A$ iff:
\[
(\forall a \in A,\; (a,a)\notin R)\;\wedge\;
(\forall a,b,c \in A,\; (a,b)\in R \wedge (b,c)\in R \to (a,c)\in R).
\]
...
\begin{remark*}[Interpretation]
A strict partial order records strict comparison directly. Irreflexivity
forbids equality-like self-comparison, while transitivity preserves chains of
strict comparisons.
\end{remark*}

\begin{remark*}[Exposition]
Irreflexivity and transitivity together force asymmetry: if $(a,b)\in R$ and
$(b,a)\in R$, transitivity gives $(a,a)\in R$, contradicting irreflexivity.
Hence every strict partial order is asymmetric, and a strict partial order can
equivalently be characterised as an asymmetric, transitive relation.
\end{remark*}

\begin{remark*}[Exposition]
Strict and non-strict orders carry the same information. Given a
\hyperref[def:partial-order]{partial order} $\leq$, the relation
$a < b \iff (a \leq b \wedge a \neq b)$ is a strict partial order; conversely,
given a strict partial order $<$, the relation
$a \leq b \iff (a < b \vee a = b)$ is a partial order. The two presentations
are interchangeable, which is why an ordered structure may be specified by
either.
\end{remark*}
...
\end{definition}
\LeanFormalizes{def:total-order}{lra-lean}{LRA.VolumeI.Order.Total}{TotalOrder}{pending}
\end{definitionbox}
...
\begin{remark*}[Predicate reading]
\[
  \operatorname{TotalOrder}(R,A)
  \iff
  \operatorname{PartialOrder}(R,A)
  \wedge \operatorname{TotalRelation}(R,A).
\]
\end{remark*}

\begin{remark*}[Interpretation]
A total order is a partial order with comparability for every pair of
elements. It rules out incomparable pairs while retaining antisymmetry and
transitivity.
\end{remark*}
