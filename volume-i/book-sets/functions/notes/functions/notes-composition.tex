...
\[
  \operatorname{AssociativeComposition}(f,g,h)
\] means that the two possible
...
\begin{remark*}[Predicate reading]
\[
  \operatorname{IdentityNeutralForComposition}(f,\id_A,\id_B)
\] means that the
...
\] and $\operatorname{Injective}(g)$ imply
...
\begin{figure}[htbp]
\centering
\input{volume-i/book-sets/functions/notes/functions/figure-one-sided-inverses}
...
\begin{remark*}[Predicate reading]
\[
  \exists g\,\operatorname{LeftInverse}(g,f)
\] is equivalent to
...
\subsubsection*{Basic Preimage Laws}

\begin{theorembox}{Theorem (Basic Preimage Laws)}
\begin{theorem}[Basic Preimage Laws]\label{thm:basic-preimage-laws}
...
\]
\hyperref[prf:basic-preimage-laws]{Go to proof.}
\end{theorem}
\end{theorembox}
...
f^{-1}(S\triangle T)=f^{-1}(S)\triangle f^{-1}(T),\\
\id_A^{-1}(U)=U,\quad
(g\circ f)^{-1}(R)=f^{-1}(g^{-1}(R)),\quad
S\subseteq T\to f^{-1}(S)\subseteq f^{-1}(T).
\end{gathered}
\]
\end{remark*}

\begin{remark*}[Interpretation]
These are the tactical preimage identities used constantly in topology and
measure theory.  The key habit is to reduce every statement to the single
membership equivalence
\[
a\in f^{-1}(S) \iff f(a)\in S.
\]
Once that is expanded, unions become disjunctions, intersections become
conjunctions, complements become negations, and composition becomes
substitution.
\end{remark*}
...
\begin{remark*}[Predicate reading]
\[
  \operatorname{PreservesIndexedUnion}(f,\{T_i\}_{i\in I})
\] and
...
\begin{remark*}[Predicate reading]
\[
  \operatorname{ImagePreimageAdjunction}(f,S,T)
\] records the inclusions
...
\begin{theorembox}{Theorem (Image of a Preimage)}
\begin{theorem}[Image of a Preimage]\label{thm:image-of-preimage}
...
f(f^{-1}(T))=T.
\]
\hyperref[prf:image-of-preimage]{Go to proof.}
\end{theorem}
\end{theorembox}
...
f(f^{-1}(T)) = T\cap \operatorname{im}(f),
\qquad
T\subseteq \operatorname{im}(f)\to f(f^{-1}(T))=T.
\]
\end{remark*}

\begin{remark*}[Interpretation]
Pulling $T$ back and then pushing forward recovers exactly the part of $T$
that the function can actually hit.  The operation cannot create target values
outside the image of $f$, and it cannot lose target values from $T$ that are
