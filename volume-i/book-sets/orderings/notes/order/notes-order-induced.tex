...
\[
  \operatorname{Preorder}(\leq_f,A)
\] means that the induced relation is
...
\begin{remark*}[Predicate reading]
\[
  \operatorname{PartialOrder}(\leq_f,A)
\] holds exactly when
...
\[
\operatorname{OrderEmbedding}(f,\mathsf{PartiallyOrderedSet}(A,\leq),
\mathsf{PartiallyOrderedSet}(B,\leq'))
\]
  means that
\(f\) both preserves and
reflects order.
\end{remark*}
...
\] follows from order reflection and
