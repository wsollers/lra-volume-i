...
% ---------------------------------------------------------
% Preorder and loset (defined elsewhere --- pointers only)
% ---------------------------------------------------------
\begin{remark*}[Preorder and loset are defined elsewhere]
The order relations this section uses are defined in the Relations chapter: a
\hyperref[def:preorder]{preorder} is reflexive and transitive, and a
\hyperref[def:total-order]{linear order} is a total (connex) partial order. The
corresponding \emph{structure} --- the \hyperref[def:loset]{loset}, i.e.\ the
linearly/totally ordered set --- is catalogued with the order structures in the
Ordered Sets section. Nothing is restated here, so no definition can drift.
\end{remark*}
...
% Transitive closure
% ---------------------------------------------------------
\begin{remark*}[Transitive closure (defined in the Relations chapter)]
\label{def:trans-closure}
The \emph{transitive closure} $T(R)$ of a binary relation $R$ on $X$ --- the
\hyperref[def:relation-transitive-closure]{smallest transitive relation
containing $R$} --- is defined in the Relations chapter. We recall it here only
to use it, and do not restate the definition, so the two chapters cannot drift
apart. (For the explicit construction, see the description below.)
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:relation-transitive-closure]{Transitive Closure (Relations)}
...
\label{cor:complete-preorder-extension}
...
\begin{remark*}[Proof]
\hyperref[prf:complete-preorder-extension]{Proof details.}
\end{remark*}
...
% ---------------------------------------------------------
\begin{definitionbox}{Definition and Proposition (Only Weak Cycles)}
\begin{definition}[and Proposition (Only Weak Cycles)]\label{def:owc}
...
\begin{itemize}
  \item \hyperref[def:relation]{Relation}
  \item \hyperref[def:sym-asym]{Symmetric and Asymmetric Parts}
  \item \hyperref[def:trans-closure]{Transitive Closure}
  \item \hyperref[def:preorder-extension]{Extension of a Preorder}
\end{itemize}
\end{dependencies}
